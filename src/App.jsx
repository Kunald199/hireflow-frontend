...
{resumeParsed && (
  <div className="mt-3 bg-green-50 border border-green-200 rounded-lg p-4 space-y-2">
    <p className="text-xs font-semibold text-green-700">✅ Resume Parsed</p>

    <div className="flex items-center justify-between">
      <p className="text-sm font-bold text-green-800">{resumeParsed.name}</p>
      <span className="text-xs bg-green-200 text-green-800 px-2 py-0.5 rounded-full">
        {resumeParsed.yearsOfExperience} yrs exp
      </span>
    </div>

    <p className="text-xs text-green-700">
      {resumeParsed.currentTitle} at {resumeParsed.currentCompany}
    </p>

    <p className="text-xs text-green-600 italic">{resumeParsed.background}</p>
    ...
    {resumeParsed.achievements?.length > 0 && (
      <div>
        <p className="text-xs font-semibold text-green-700 mb-1">Key Achievements</p>
        <ul className="space-y-0.5">
          {resumeParsed.achievements.slice(0, 3).map((a, i) => (
    ...
    {resumeParsed.previousCompanies?.length > 0 && (
      <p className="text-xs text-green-600">
        <strong>Previous:</strong> {resumeParsed.previousCompanies.join(', ')}
      </p>
    )}
  </div>
)}
